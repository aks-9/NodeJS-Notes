//* Document Middleware- 3 -post document middleware
...
tourSchema.post("save", function (doc, next) {
  ...
  console.log(doc); //here we actually no longer have the disk keyword, but instead we have the basically finished document here in doc.
  next();
});
...
==> indeed. So our slug is here, right, and so let's take a look at our console, and so indeed, our second pre middleware also run, so the one logging 'will save document' and then our post middleware then logged to the console the final document.


==> we can have middleware running before and after a certain event. And in the case of document middleware, that event is usually the 'save' event. And then in the middleware function itself, we have access to the 'this' keyword, which is gonna point at the currently being saved document. And it's also very important to keep in mind that this 'save' middleware only runs for the 'save' and 'create' Mongoose methods. It's not gonna run, for example, for 'insert many' and also not for 'find one' and 'update' or 'find by ID and update' 
*/
