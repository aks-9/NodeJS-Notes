...
tourSchema.virtual("durationWeeks").get(function () {
  //we need to define the get method. And that's just because this virtual property here will basically be created each time that we get some data out of the database. And so this get function here is called a getter.

  //Now in here we pass a function, and actually this call back function is gonna be a real function, so not an arrow function. This regular function here because remember, an arrow function does not get its own disk keyword. In here we actually need the disk keyword because the disk keyword in this case is going to be pointing to the current document. And so usually when we want to use this, then we should always use a regular function. So really everywhere in Mongoose, I'm gonna always be using these regular functions that we're used to.
...
we should then be able to see duration weeks. Let's wait for it, and indeed, here it is. So it's five days, and so that is 0.71 weeks.
