...
// For example, each time a new document is saved to the database, we can run a function between when the 'save' command is issued and the actual saving of the document, or also after the actual saving. And that's the reason why Mongoose middleware is also called 'pre' and 'post' hooks.

// there are four types of middleware in Mongoose: document, query, aggregate, and model middleware.

// And in this lecture, we're gonna talk about document middleware, which is middleware that can act on the currently processed document.
...
// we define a middleware on the schema, like 'tourSchema.pre'. And so this is for 'pre' middleware, which is gonna run before an actual event. And that event in this case is the 'save' event. And so this call back function that we're gonna define here next, will be called before an actual document is saved to the database. So it runs before the save command and the '.create' command. But not on 'insertMany'. So if we use 'insertMany', then that will actually not trigger the 'save' middleware.
