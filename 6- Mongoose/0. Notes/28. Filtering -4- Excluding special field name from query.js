...
exports.getAllTours = async (req, res) => {
  try {
    ...
    //Now, the problem with this implementation, is that its actually way too simple. That's because, later on, we will have other query parameters. For example, 'sort' for sorting functionality, or 'page' for pagination. We need to make sure that we are not querying for these in our database. For example, if we added 'page=2' in our query, then we would not get any result because there is no document in this collection where 'page=2'. We only want to use this parameter 'page', to implement pagination and not to actually 'query' in the database. So what we will have to do is, to basically exclude these special field names from our query string before we actually do the filtering.
