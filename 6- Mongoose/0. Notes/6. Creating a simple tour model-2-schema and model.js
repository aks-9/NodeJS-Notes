...
const tourSchema = new mongoose.Schema({
  name: {
    type: String,
    required: [true, "A tour must have a name"], // here in the 'required' we can actually specify the error message that we want to be displayed when we're missing this 'name' field. So, in order to do that we just have to pass in an array, and the first element is 'true', and the second one is the 'error message' string. This is known as a 'validator'.
  ...
  rating: {
    type: Number,
    default: 4.5, // if we now create a new tour document using this schema and not specifying the rating, it would then automatically be set to 4.5.
  },
});
