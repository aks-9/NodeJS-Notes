...
exports.getTourStats = async (req, res) => {
  try {
    ...
    //And for that, we pass in an 'array' of so-called 'stages'. And again, the documents then pass through these 'stages' one by one, step by step in the sequence as we define it here.
    ...
    const stats = await Tour.aggregate([
      ...
      },
      {
        ...
        $group: {
          ...
          numTours: { $sum: 1 }, //we basically add 1 to this 'numTours' counter, for each document that's gonna go through this pipeline.
