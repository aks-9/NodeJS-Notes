...
const getDogPic = async () => {
  try {
    ...
    const images = all.map((element) => element.body.message); //So using 'map' method on 'all' array, we will loop through the 'all' array and save the value that we return in each iteration, into a new array called 'images'.  And the current value in each iteration is called 'element' and what we will return is element.body.message.
