...
//The most important ones are the 'development' and the 'production' environment. But there are other environments that bigger teams might use. So this type of settings like different databases or logging turned on or off, that will be based on environment variables. Now by default, Express sets the environment to 'development'.
...
//So in summary, environment variables are global variables that are used to define the environment in which a node app is running. Now this 'env' variable here is actually set by Express, but node JS itself actually also sets a lot of environment variables that are located at 'process.env'.
