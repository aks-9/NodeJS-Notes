...
//with a post request, data is sent from the client to the server in 'body'. This data should be then ideally available on the request object. But Express does not put that data on the 'body' to on the request, and in order to have that data available, we have to use something called 'middleware'.

app.use(express.json()); //middleware  is basically a function that can modify the incoming request data. It's called middleware because it stands in between the request and the response. It's just a step that the request goes through while it's being processed. And in this step, the data from the 'body' is added to the request object by using this middleware.
